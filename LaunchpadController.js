window.LaunchpadInstance = function(n,callback){
	...
	self.setOutput = function(output){
		...
		if(isX) output.send([240,0,32,41,2,12,0,127,247]);
       	...
       	$name.text(output.name.substr(0,15));
	...
	self.setInput = function(input){
		...
		$name.text(input.name.substr(0,15));
	}
...
window.LaunchpadController = function(onPianoMessage){

	const COUNT = 1;

	let self = this;
	self.$el = $('<launchpadcontroller>');
	...
	function reviseMidi(e) {
        ...
        for (var input of midiAccess.inputs.values()){
			...
			if(input.name == 'USB-MIDI' || input.name == 'minilogue xd KBD/KNOB'){
				console.log('here we go')
				input.onmidimessage = onPianoMessage;
			}
        }
